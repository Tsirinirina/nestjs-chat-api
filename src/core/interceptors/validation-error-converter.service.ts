...
export class ValidationErrorConverterService {
  ...
  public static convert(validationErrorList: ValidationError[]): string {
    return this.getValidConstraints(validationErrorList)
      .map((constraints) =>
        ValidationErrorConverterService.convertObjectToValueArray(constraints),
      )
   ...
   * @private
   */
  private static convertObjectToValueArray(object: {
    [x: string]: string;
  }): string[] {
   ...
   * @private
   */
  private static getValidConstraints(
    ...
    return validationErrorList.reduce<{ [x: string]: string }[]>(
      (constraints, validationError) => {
        ...
        if (validationError.children && validationError.children.length) {
          return [
            ...constraints,
            ...this.getValidConstraints(validationError.children),
          ];
        }

        return [
          ...constraints,
          { error: 'Unknown validation error constraints!' },
        ];
      },
      [],
    );
